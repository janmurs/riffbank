...
async function init() {
  initAppContext({ render, navigateForward, goBack, setHeader, syncTabs, getActiveScreenEl: () => activeScreenEl, _updateNotifBadge, resetSongsFilters, getSongsListState: () => songsListState, getProjectsOwnerFilter: () => projectsOwnerFilter, setProjectsOwnerFilter: (v) => { projectsOwnerFilter = v; }, openCreateOverlay, openSheet, openSongMenu, playVersion, addToQueue, createVersion, setActive, openSongFilters, pickAudioFile, getGlobalAudio: () => globalAudio, unlockAudioOnce, playNowPlaying, syncMiniPlayerUI, shareInviteSong, openVersionMenu, nav });
